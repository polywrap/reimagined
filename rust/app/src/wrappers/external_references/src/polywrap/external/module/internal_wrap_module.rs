...
impl InternalModule for InternalWrapModule {
    fn receive(&self, buffer: &[u8], external_module: Arc<dyn ExternalModule>) -> Vec<u8> {
        ...
        match resource {
            x if x == InternalResource::InvokeGlobalFunction as u32 => invoke_global_function(data_buffer, external_module),
            x if x == InternalResource::InvokeClassMethod as u32 => invoke_class_method(data_buffer, external_module),
